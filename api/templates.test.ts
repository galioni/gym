...
vi.mock("./_lib/cloudSyncApiService.js", () => ({
  CloudSyncApiService: vi.fn(function() {
    return {
      getResource: vi.fn().mockResolvedValue({ version: 1, updatedAt: "2026-01-01T00:00:00Z", templates: {} }),
...
// templates: {} passes hasValidTemplateKeys (no keys to fail the regex)
const VALID_PUT_BODY = { version: 1, updatedAt: "2026-01-01T00:00:00Z", templates: {} };
const PUT_HEADERS = { "x-forwarded-for": "1.2.3.4", "content-type": "application/json" };

let ipCounter = 0;
function uniqueIp() { return `10.2.${Math.floor(++ipCounter / 255)}.${ipCounter % 255}`; }
...
describe("GET /api/templates", () => {
  ...
  it("returns 200 with payload on GET happy path", async () => {
    mockRequireAuth.mockResolvedValue(PRO_AUTH);
    const req = createMockRequest({ method: "GET", headers: { "x-forwarded-for": uniqueIp() } });
    const { res, state } = createMockResponse();
    await handler(req, res);
    expect(state.statusCode).toBe(200);
    expect((state.jsonPayload as { templates: unknown }).templates).toBeDefined();
  });

  it("returns 404 when no stored templates exist", async () => {
...
describe("PUT /api/templates", () => {
  ...
  it("returns 400 for invalid PUT payload", async () => {
    ...
    const req = createMockRequest({
      ...
      body: { version: 1, updatedAt: "2026-01-01T00:00:00Z" }, // missing "templates"
    });
    const { res, state } = createMockResponse();
    await handler(req, res);
    expect(state.statusCode).toBe(400);
    expect((state.jsonPayload as { error: string }).error).toMatch(/Invalid template payload/);
  });

  it("returns 400 for PUT with invalid template key format", async () => {
    mockRequireAuth.mockResolvedValue(PRO_AUTH);
    const req = createMockRequest({
      method: "PUT",
      headers: { ...PUT_HEADERS, "x-forwarded-for": uniqueIp() },
      // Key with uppercase fails SESSION_TYPE_KEY_RE
      body: { version: 1, updatedAt: "2026-01-01T00:00:00Z", templates: { "Invalid-Key": {} } },
    });
    const { res, state } = createMockResponse();
    await handler(req, res);
    expect(state.statusCode).toBe(400);
  ...
  it("accepts AI-generated snake_case template keys", async () => {
    mockRequireAuth.mockResolvedValue(PRO_AUTH);
    const req = createMockRequest({
      method: "PUT",
      headers: { ...PUT_HEADERS, "x-forwarded-for": uniqueIp() },
      body: { version: 1, updatedAt: "2026-01-01T00:00:00Z", templates: { full_body: {}, rest_day: {} } },
    });
    const { res, state } = createMockResponse();
    await handler(req, res);
    expect(state.statusCode).toBe(200);

...
vi.mock("./_lib/cloudSyncApiService.js", () => ({
  CloudSyncApiService: vi.fn(function() {
    return {
      getResource: vi.fn().mockResolvedValue({ version: 1, updatedAt: "2026-01-01T00:00:00Z", data: {} }),
...
import handler from "./workout-data";
...
const VALID_PUT_BODY = { version: 1, updatedAt: "2026-01-01T00:00:00Z", data: {} };
...
describe("GET /api/workout-data", () => {
  ...
  it("returns 200 with payload on GET happy path", async () => {
    ...
    expect((state.jsonPayload as { data: unknown }).data).toBeDefined();
  });

  it("returns 404 when no stored data exists", async () => {
...
describe("PUT /api/workout-data", () => {
  ...
  it("returns 400 for invalid PUT payload", async () => {
    ...
    const req = createMockRequest({
      ...
      body: { version: 1, updatedAt: "2026-01-01T00:00:00Z" }, // missing "data"
    });
    const { res, state } = createMockResponse();
    await handler(req, res);
    expect(state.statusCode).toBe(400);
    expect((state.jsonPayload as { error: string }).error).toMatch(/Invalid workout payload/);
  });

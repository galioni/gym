    ...
    <header ref={menuRef} className="sticky top-0 z-40 border-b border-white/10 bg-background/80 backdrop-blur-xl motion-sweep">
      <div className="max-w-4xl mx-auto px-4 py-3 md:py-4">
        <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-4 bg-surface/60 border border-white/10 rounded-2xl px-3 py-2">
                 ...
                 <div className="flex gap-1 ml-1 pl-3 border-l border-white/10">
                    <Button onClick={exportBackup} size="icon" variant="ghost" title="Export Data" className="h-11 w-11">
                        <Download size={14} />
                    </Button>
                 ...
                 <div className="flex flex-col gap-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.14em]">Theme</label>
                    <select
                      value={theme}
        ...
        <div className={cn(
            ...
            isOpen ? "max-h-[460px] opacity-100 mt-4 border-t border-white/10 pt-4" : "max-h-0 opacity-0"
        )}>
            <div className="space-y-4 pb-2 bg-surface/50 border border-white/10 rounded-2xl p-3">
                 ...
                 <div className="space-y-1">
                    <label className="text-xs text-slate-500 font-medium uppercase tracking-[0.14em]">Theme</label>
                    <select
                      value={theme}
                      onChange={(event) => onThemeChange(event.target.value as AppTheme)}
                      className="w-full bg-background/80 border border-white/10 rounded-xl px-3 py-2 text-sm text-slate-200 focus:ring-2 focus:ring-primary/50 outline-none appearance-none"
                ...
                <div>
                     <Button onClick={() => { exportBackup(); setIsOpen(false); }} variant="secondary" className="w-full min-h-11 gap-2 text-xs">
                       <Download size={14} />
                       Backup Data
                    </Button>
                 </div>
